...
export const serviceProvider = async (req, res) => {
  try {
    ...
    if (existProvider) {
      ...
      res.status(200).json({status:200, message: "Provider Updated Personal Information successfully" });
    } else {
...
export const updateProviderVerified = async (req, res) => {
  try {
    ...
    const updated = await providerService.updateVerifiedProvider({ service_provider_id,  first_name,last_name,email,mobile_no });
    res.status(200).json({ status:200,message: 'Provider updated successfully', data: updated });
  } catch (error) {
